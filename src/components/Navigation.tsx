...
const Navigation = ({ activeSection, setActiveSection }: NavigationProps) => {
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <nav className="hidden md:block fixed left-8 top-1/2 transform -translate-y-1/2 z-20">
      ...
      {/* Mobile Navigation */}
      <nav className="md:hidden fixed top-8 left-1/2 transform -translate-x-1/2 z-20">
        <div className="flex space-x-2 bg-black/50 backdrop-blur-md rounded-full px-2 py-2">
          {sections.map((section) => (
            <button
              key={section.id}
              onClick={() => setActiveSection(section.id)}
